...
def mock_operation(fail=False, fail_times=0, success_after=0):
    ...
    if fail_times > 0 and mock_operation.call_count <= fail_times:
        raise ValueError(f"Failed intentionally ({mock_operation.call_count}/{fail_times})")
    if success_after > 0 and mock_operation.call_count <= success_after:
        raise ValueError(f"Failed intentionally until call {success_after + 1}")
    return "Success"
...
class TestCircuitBreaker(unittest.TestCase):
    ...
    def test_success_remains_closed(self):
        cb = CircuitBreaker(3, 1)
        [self.assertEqual(cb.execute(mock_operation), "Success") for _ in range(5)]
        self.assertEqual(cb.state, CircuitBreakerState.CLOSED)
        self.assertEqual(cb.failure_count, 0)

    def test_transition_to_open(self):
        cb = CircuitBreaker(2, 10)
        self.assertRaises(ValueError, cb.execute, mock_operation, fail=True)
        self.assertEqual(cb.state, CircuitBreakerState.CLOSED)
    ...
    def test_open_state_rejects_calls(self):
        cb = CircuitBreaker(1, 10)
        self.assertRaises(ValueError, cb.execute, mock_operation, fail=True)
        self.assertEqual(cb.state, CircuitBreakerState.OPEN)
        self.assertRaises(CircuitBreakerOpenException, cb.execute, mock_operation)

    def test_transition_to_half_open(self):
        rt = 0.1
        cb = CircuitBreaker(1, rt)
        self.assertRaises(ValueError, cb.execute, mock_operation, fail=True)
        self.assertEqual(cb.state, CircuitBreakerState.OPEN)
        time.sleep(rt * 1.1)
        self.assertTrue(
            cb.last_failure_time
            and (time.monotonic() - cb.last_failure_time) > cb.recovery_timeout_seconds
        )
        mock_operation.call_count = 0
        result = cb.execute(mock_operation)
    ...
    def test_reset_after_success_in_closed(self):
