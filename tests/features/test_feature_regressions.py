...
class TestFeatureRegressions(unittest.TestCase):
    def test_monitoring_serves_metrics_and_health(self):
        ...
        self.assertIn(f'# HELP {prefix}_proxy_request_duration_seconds Proxy request duration seconds', metrics_body)
        self.assertIn(f'# HELP {prefix}_proxy_request_phase_duration_seconds Proxy request phase duration seconds', metrics_body)
    ...
    def test_proxy_manager_request_response_mode_returns_payload(self):
        ...
        with patch('akita_ares.features.proxying.Identity.recall', return_value=SimpleNamespace()), \
            ...
            request_id = manager.send_via_proxy(
                'abcdef1234567890abcdef1234567890',
                b'data',
                response_callback=lambda payload, error: (payloads.append(payload), errors.append(error)),
                target_destination_name='app_name.service_behind_firewall.data_service',
                request_path='/status',
            )
        ...
        self.assertTrue(any(item[:4] == ('default', 'request', 'proxy_link_setup', 'success') for item in metrics.proxy_request_phase_durations))
        self.assertTrue(any(item[:4] == ('default', 'request', 'proxy_roundtrip', 'success') for item in metrics.proxy_request_phase_durations))
    ...
    def test_proxy_manager_prefers_most_specific_matching_route(self):
        ...
        route, error = manager._select_proxy_route(None, 'app_name.service_behind_firewall.data_service')
    ...
    def test_proxy_manager_forwards_request_via_target_link(self):
        ...
        manager._build_outbound_destination = lambda target_hash, target_name: (SimpleNamespace(hash=b'\xaa' * 16, hexhash='aa' * 16), None)
        ...
        self.assertEqual(metrics.proxy_request_outcomes[-1], ('proxy_node_service', 'request', 'success'))
        self.assertEqual(metrics.proxy_request_durations[-1][:3], ('proxy_node_service', 'request', 'success'))
        self.assertGreaterEqual(metrics.proxy_request_durations[-1][3], 0)
        self.assertTrue(any(item[:4] == ('proxy_node_service', 'request', 'target_link_setup', 'success') for item in metrics.proxy_request_phase_durations))
        self.assertTrue(any(item[:4] == ('proxy_node_service', 'request', 'target_request_service', 'success') for item in metrics.proxy_request_phase_durations))
